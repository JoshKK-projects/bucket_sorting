...
**Assumptions**
        Assumes there is a *,*,* bucket and its the second one, if it was going to be variable i would either just iterate to find it and get its index or instead of 
        finding buckets by an assigned int index i would just use the bucket col names concat'ed together to find them
        Files dont have trailing \n

**Design**
        simple script in bucket_sorting.py. It uses the little module of buckersorter.py to call its functions. The script itself does put
        together the json and writes it to file. Putting the bit that creates the json could just as easily go in the little module but
        in the end I decided to leave it there just to make it more clear what the script does. The script is wrapped in a function just to make
        it easier to sub in various csv and output file names
**Other Notes**
        output writes to output.txt. I just used sublime's JSON format to make it human readable.
        test_output.txt is a test output with test buckets and purchases i worked out beforehand. the test purchases have an
        extra col for the bucket they are supposed to go into. Given more time a more thorough way to test might be to come up with 
        some test purchases, then write a small script to make a bucket for each possible permutation of the buckets and make sure they 
        end up in the right buckets with every possible-possible bucket as an option.
